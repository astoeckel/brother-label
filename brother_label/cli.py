...
@click.option(
    "-m",
    "--device",
    type=click.Choice(BrotherDeviceManager().keys()),
    envvar="BROTHER_LABEL_DEVICE",
)
@click.option("-p", "--target", metavar="TARGET", envvar="BROTHER_LABEL_TARGET")
...
@click.argument(
    "images", nargs=-1, type=click.File("rb"), metavar="IMAGE [IMAGE] ..."
)
...
@click.option(
    ...
    help="The label (size, type - die-cut or endless). Run `brother_label info labels` for a full list including ideal pixel dimensions.",
)
@click.option(
    "-r",
    "--rotate",
...
def create_cmd(ctx, no_compress, no_cut, *args, **kwargs):
    ...
    brother = BrotherLabel(ctx.meta.get("DEVICE"), strict=True)
    instructions = brother.convert(
        compress=not no_compress, cut=not no_cut, **kwargs
    )
...
def print_cmd(ctx, no_compress, no_cut, *args, **kwargs):
    ...
    brother.print(
        blocking=True, compress=not no_compress, cut=not no_cut, **kwargs
    )
...
def send_cmd(ctx, *args, **kwargs):
    ...
    brother.send(kwargs["instructions"].read(), blocking=True)
