...
class BrotherLabel(object):
    ...
    def convert(self, type, images, device=None, **kwargs):
        ...
        return self.converter.convert(
            device or self.device, type, images, **kwargs
        )
    ...
    def print(
        self, type, images, target=None, backend=None, blocking=True, **kwargs
    ):
        instructions = self.convert(type, images, **kwargs)

        self.send(
            instructions, blocking=blocking, target=target, backend=backend
        )
    ...
    def send(self, instructions, target=None, backend=None, blocking=True):
        ...
        backend = (
            backend_factory(backend)["backend_class"]
            if backend
            else self.backend
        )
